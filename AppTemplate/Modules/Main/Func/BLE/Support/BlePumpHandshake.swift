//
//  BlePumpHandshake.swift
//  AppTemplate
//
//  Pump 配网/协议握手：F0 解析 productType → Profile；F0 加密字段 → FD 是否实发。

import Foundation
import AppStart

/// 产品典型配网链；FD 是否实发以 F0 解析为准。
enum BlePumpProvisionProfile {
    /// CONNECT_F0
    case f0
    /// CONNECT_F0_FD
    case f0_fd
    ...
    /// F0 CAB productType（= 广播 byte[1]）→ 配网链。
    static func resolve(productType: UInt8) -> BlePumpProvisionProfile {
        switch productType {
        case 0x10, 0x11:
            return .f0
        case 0x01, 0x03, 0x04:
...
struct BlePumpHandshakeResult {
    var profile: BlePumpProvisionProfile?
...
enum BlePumpHandshake {
    ...
    ) async throws -> BlePumpHandshakeResult {
        var result = BlePumpHandshakeResult(sentFD: false)

        let f0Info = try await sendF0(connection, log: log)
        result.f0Info = f0Info

        let profile = BlePumpProvisionProfile.resolve(productType: f0Info.productType)
    ...
    private static func sendF0(
        ...
        guard let ack = try await write(BlePumpCommand.f0Auth(), on: connection, log: log) else {
            throw BlePumpHandshakeError.missingAck("F0")
        }
    ...
    private static func sendFDIfNeeded(
        ...
        guard let ack = try await write(
            ...
            throw BlePumpHandshakeError.missingAck("FD")
        }
    ...
    private static func sendF7(
        ...
        guard let ack = try await write(BlePumpCommand.f7Triplet(), on: connection, log: log) else {
            throw BlePumpHandshakeError.missingAck("F7")
        }
